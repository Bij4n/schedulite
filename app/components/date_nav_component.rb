class DateNavComponent < ViewComponent::Base
  ...
  def initialize(dates:, selected_date:, day_counts:, view_mode:, provider_id: nil)
    @dates = dates
  ...
  def call
    tag.div(class: "border-b border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50") do
      tag.div(class: "mx-auto max-w-5xl px-4 sm:px-6") do
  ...
  def nav_params(overrides = {})
    { date: @selected_date.iso8601, view: @view_mode, provider_id: @provider_id }.compact.merge(overrides)
  end
  ...
  def week_nav
    tag.div(class: "flex items-center justify-between py-2") do
      ...
      safe_join([
        link_to(helpers.root_path(**nav_params(date: prev_week.iso8601)), class: "p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300") do
        ...
        link_to(helpers.root_path(**nav_params(date: next_week.iso8601)), class: "p-2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300") do
          tag.svg(xmlns: "http://www.w3.org/2000/svg", fill: "none", viewBox: "0 0 24 24", stroke_width: "2", stroke: "currentColor", class: "w-4 h-4") do
  ...
  def day_cell(date)
    ...
    link_to(helpers.root_path(**nav_params(date: date.iso8601)), class: "#{base} #{colors}") do
      safe_join([
  ...
  def toggle_button(label, mode)
    ...
    link_to(label, helpers.root_path(**nav_params(view: mode)), class: classes)
  end
end
